            ...
            case "cat":
                ...
                switch (textOne) {
                    ...
                    case "/rules for getting to know a pet":
                        bot.sendMessage(chatId, """
                                  Преимущества обращения в приют за питомцем.\s
                    ...
                    case "/documents for registration of guardianship": {
                        ...
                        bot.sendMessage(chatId, """
                                ...
                                Мы  всегда благодарны вам за фотоотчеты "\s""");
                    }
                    bot.sendMessage(bot.getProjectKeyboardConverter().inLineKeyboard(chatId,
                            "Выберете пожалуйста, этот пункт, что бы вернуться, обратно в меню!",
                            TheReturnToKeyboardButton.getList("/how to take a pet from a shelter" + Animals.CAT.getTitle()), bot));
                    ...
                    case "/reasons for abandoning a pet": {
                        // обработчик при этом условии
                        bot.sendMessage(chatId, """
                                -  Большое количество животных дома\s
                                 -  Нестабильные отношения в семье\s
                                 -  Наличие маленьких детей\s
                                 -  Съемное жилье\s
                                 -  Животное в подарок или для работы                        \s
                    ...
                    case "/animal":
                        ...
                        bot.sendMessage(chatId,
                                """
                                        ...
                                        \s
                                        Поэтому, заводя питомца, необходимо оценить все возможные риски и подготовить свой дом для нового обитателя. Итак, все, чем можно отравиться, прячем подальше. Бытовая химия,порошки, шампуни, дезинфицирующие средства, лекарства должны быть вне зоны досягаемости котенка. Все, что может разбиться и ранить осколками, а также все, что сердцу дорого, тоже должна постигнуть та же участь. Уделите особое внимание вашим комнатным растениям, может статься, что некоторые из них содержат ядовитый сок в стеблях и листьях, это не страшно для человека, но опасно для кошки, которая, возможно, захочет поживать зелень.   Компьютерные, телевизионные и телефонные провода нужно тоже спрятать от острых зубов котят. Для этих целей существуют специальные пластиковые короба и трубки, которые пускаются либо вдоль плинтуса, либо поднимаются под потолок.Прежде чем котенок или взрослая кошка, впервые попавшая в дом, начнет царапать мебель и стены, нужно приобрести когтеточку – высокую, чтобы взрослая особь могла встать на задние лапки и вытянуться во весь рост. Обычно кошки привыкают к своему тренажеру и не покушаются на другие уголки квартиры. Если все же был страстно облюбован угол дивана и никакие отпугиватели не помогают, то придется смириться и оббить этот угол плотным материалом  или приладить когтеточку к нему.
                    ...
                    case "/an adult animal": {
                        ...
                        bot.sendMessage(chatId, """
                                ...
                                Купите заранее миски для еды и воды. Главное, что стоит учитывать при их выборе – чтобы кошке было удобно ими пользоваться. Специалисты советуют приобрести мисочки трех видов: для влажного корма – неглубокую, для сухого корма – глубокую, а также устойчивую глубокую миску для воды. Пластмассовая посуда менее гигиеничная, чем стеклянная, металлическая или керамическая, поэтому стоит отдать предпочтение этим материалам.

                                Что касается корма, стоит сказать, что резкая смена привычного питания – это тоже стресс для организма. Если вы берете котенка от заводчика, он подскажет вам, к какому корму приучено животное. Если вы решили взять кошку из приюта, вы также можете попробовать узнать, чем кормили кошку там. Если вас по каким-то причинам не устраивает уже привычная для вашего питомца еда и вы хотите перевести животное на корм другой марки, делайте это постепенно. Специалисты считают, что стоит кормить кошку как раньше неделю или две, а после добавить четверть желаемого корма от порции в привычный рацион с прибавкой 10% нового корма каждый день.
                    ...
                    case "/how to properly submit a report": {
                        ...
                        bot.sendMessage(chatId, """
                                ...
                                \s
                                Отчет нужно присылать каждый день, ограничений в сутках по времени сдачи отчета нет. Каждый день волонтеры осматривают все присланные отчеты после 21:00.
                                \s
                                Для создания и отправки отчета,
                                 выберите кнопку в меню ->   Начать отчет\s
                                 и пошагово, введите предложенные Вам позиции\s""");
                        bot.sendMessage(bot.getProjectKeyboardConverter().inLineKeyboard(chatId,
                                "Выберете пожалуйста, этот пункт, что бы вернуться, обратно в меню!",
